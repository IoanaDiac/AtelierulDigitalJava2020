...
public class Main {
    public static  void main(String[] args){
        ...
        System.out.print("Studio names that have published more than 2 movies: ");
        ...
        System.out.println();
        System.out.print("Studio names in witch plays the actor with name 'actor cu 2 oscaruri': ");
        for (Studio studio : studioDatabase) {
            boolean found = false;
            for(Film film : studio.getFilme()){
                for (Actor actor : film.getActor()){
                    if(actor.getNume().equals("actor cu 2 oscaruri")) found=true;
                }
            }
            if(found)
                System.out.print(studio.getNume() + ", ");
        ...
        System.out.println();
        System.out.print("Studio names in witch plays at least an actor with age above 50: ");
        for (Film film : filmDatabase) {
            boolean found=false;
            for (Actor actor : film.getActor()){
                if(actor.getVarsta()>50) found=true;
                }
            if(found)
